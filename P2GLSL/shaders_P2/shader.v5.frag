...
vec3 shade()
{
	...
	//Atenuacion
	float d = length(p);					//Calculamos la longitud a la que está la posición del objeto de la cámara
	at = min(1.0/(c1+c2*d+c3*d*d),1.0);		//Formula de la atenuación
	c *= at;
