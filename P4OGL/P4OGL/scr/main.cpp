...
void resizeFBO(unsigned int w, unsigned int h)
{
	...
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_MIRRORED_REPEAT); // Efecto espejo
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_R32F, w, h, 0,
		GL_RGBA, GL_FLOAT, NULL);
	//Parametros
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR); //Linear para que la transicion sea suave
	...
	glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT4, //Asignar un attachment (Pasar la textura al FBO) Despues se le asigna el render target
		GL_TEXTURE_2D, colorBuffTexId, 0); // Enlaza la imagen 0

	glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT2, //Asignar un attachment (Pasar la textura al FBO) Despues se le asigna el render target
		GL_TEXTURE_2D, zBuffTexId, 0); // Enlaza la imagen 0
...
void renderFunc()
{
	...
	//Activar textura de profundidad en el puerto 1
	glActiveTexture(GL_TEXTURE0+1);
	glBindTexture(GL_TEXTURE_2D, zBuffTexId);
